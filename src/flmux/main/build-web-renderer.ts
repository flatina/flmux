...
export async function buildWebRenderer(projectRoot: string, outputDir: string): Promise<void> {
  ...
  const wsRpcPath = resolve(projectRoot, "src/flmux/renderer/transport/ws-rpc.ts");
  ...
  const result = await Bun.build({
    entrypoints: [join(projectRoot, "src/flmux/renderer/index.ts")],
    outdir: outputDir,
    target: "browser",
    minify: false,
    sourcemap: "inline",
  ...
  if (!result.success) {
    ...
    // Copy index.html
  copyFileSync(
    join(projectRoot, "src/flmux/renderer/index.html"),
