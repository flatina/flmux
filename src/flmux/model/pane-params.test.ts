...
describe("isPaneKind", () => {
  test("accepts valid kinds", () => {
    for (const kind of ["terminal", "browser", "editor", "explorer", "view"]) {
...
describe("createPaneParams", () => {
  ...
  test("creates terminal params with startup commands", () => {
    const params = createPaneParams("terminal", { startupCommands: ["echo hi"] });
    expect(params.startupCommands).toEqual(["echo hi"]);
  });
