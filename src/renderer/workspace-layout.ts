...
import type { BrowserPaneInfo, PaneSummary, TabSummary } from "../shared/app-rpc";
import { asPaneId, asTabId, type PaneId, type TabId } from "../shared/ids";
import { getDefaultPaneTitle, isPaneParams, type PaneParams } from "../shared/pane-params";
import { browserTitleFromUrl, normalizeBrowserUrlValue, panelToSummary } from "./helpers";
import type { TabRenderer } from "./tab-renderer";
...
export function collectWorkspaceBrowserPaneInfos(
  dockview: DockviewApi | null,
  tabRenderers: Map<string, TabRenderer>,
  browserPaneInfos: ReadonlyMap<PaneId, BrowserPaneInfo>
): BrowserPaneInfo[] {
  ...
  forEachWorkspacePane(dockview, tabRenderers, ({ paneId, tabId, title, params }) => {
    ...
    const cached = browserPaneInfos.get(paneId);
    if (cached) {
      panes.push(cached);
      return;
    }
